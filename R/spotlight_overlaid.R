#' Overlay a Spotlight Analysis Plot
#'
#' Conduct a spotlight analysis for a 2 x Continuous design and
#' overlay the plot.
...
#' @param simp_eff_text_offset_perc description not ready
...
#' simp_eff_text_offset_perc = 4,
#' x_axis_breaks = seq(15, 22, 1))
#' }
#' @export
spotlight_overlaid <- function(
  data = NULL,
  iv_name = NULL,
  dv_name = NULL,
  mod_name = NULL,
  ...
  y_axis_breaks = NULL,
  y_axis_limits = NULL,
  x_axis_space_left_perc = 0.1,
  x_axis_space_right_perc = 0.3,
  ...
  focal_value <- y1_error_bar_x <- y2_error_bar_x <- NULL
  ...
  if (is.null(error_bar_offset)) {
    error_bar_offset_half <- x_range * (error_bar_offset_percent / 2) / 100
  } else {
  ...
  if (is.null(simp_eff_bracket_offset)) {
    simp_eff_bracket_offset <- x_range * simp_eff_bracket_offset_perc / 100
  }
  ...
  dt2[, simp_eff_bracket_x_end := focal_value +
        error_bar_offset_half + simp_eff_bracket_offset * 2]
  # add x coordinates of simple effect text
  if (is.null(simp_eff_text_offset)) {
    simp_eff_text_offset <- x_range * simp_eff_text_offset_perc / 100
  }
  dt2[, simp_eff_text_x := simp_eff_bracket_x_end + simp_eff_text_offset]
  # add y coordinates of simple effect text
  dt2[, mean_of_y1_and_y2 := rowMeans(dt2[, c("y1", "y2"), with = FALSE])]
  # begin plotting
  # ggplot base
  ...
  if (interaction_p_include == TRUE) {
    ...
    if (is.null(interaction_p_value_x)) {
      if (case == 1) {
        interaction_p_value_x <- focal_values[2]
      } else {
  ...
  # edit axes
  if (case == 1 & observed_dots == FALSE & reg_lines == FALSE) {
    x_axis_breaks <- focal_values
    tick_marks_range <- diff(range(focal_values))
    x_axis_lower_limit <- utils::head(focal_values, 1) -
      tick_marks_range * x_axis_space_left_perc
    x_axis_upper_limit <- utils::tail(focal_values, 1) +
      tick_marks_range * x_axis_space_right_perc
    x_axis_limits <- c(x_axis_lower_limit, x_axis_upper_limit)
    x_axis_tick_mark_labels <- focal_value_description
  }
  # edit x axis
