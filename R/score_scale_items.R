...
#' @param mean logical. If \code{mean = TRUE}, mean of the scale items
#' will be calculated (default = TRUE).
#' @param sum logical. Input for the argument \code{mean} takes precedence.
#' If \code{mean = FALSE, sum = TRUE}, sum of the scale items will
...
#' reverse = list(c(5, 3)), reverse_code_minuend = 6)
#' @export
#' @import data.table
score_scale_items <- function(
  ...
  # mean takes precedence over sum
  if (mean == TRUE & sum == TRUE) {
    sum <- FALSE
    message(paste0(
      'Although both the "mean" and "sum" arguments were set to be ',
      "TRUE, only the mean will be calculated."))
  }
